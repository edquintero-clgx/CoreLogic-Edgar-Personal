...
DECLARE words_to_remove STRING;
SET test_clip = '7981047294';-- '2327788344'; '1010729008';
SET words_to_remove = r'\b(airport|international airport|the|limited|liability|llc|inc|incorporated|corp|shoppingcntr|center|plaza|limited|ltd|company|co|shop|shopping|center|church|co|corporation)\b';


CREATE OR REPLACE TABLE `clgx-idap-bigquery-int-278e.stg_default_dataset.eq_efx_fuzzy_matching` AS 

WITH db_usa AS (
            SELECT 
        'db_usa' AS source_table,
        database_individual_id as rec_id,
        CONCAT(company,'db_usa',NULLIF(CASE 
        ...
        NULLIF(LOWER(REGEXP_REPLACE(REGEXP_REPLACE(LOWER(company), words_to_remove, ''),r'[^\w]', '')),'') AS clean_company,
        CASE 
            WHEN naics_1 IN (
            ...
            WHEN individualfirm_code = '2' THEN 'Y'
            WHEN professionalamount IN ('B','C','D','E','F') THEN 'Y'
            WHEN professionalflag = 'P' THEN 'Y'
    ...
    FROM `clgx-idap-bigquery-int-278e.land_property_commercial_prefill.cp_firmographics_dbusa_clip_v2_1`
...
echo_analytics AS (
    SELECT
        ...
        CONCAT(poi_name,'echo_analytics',NULLIF(CASE 
        ...
        address_id,
        clip_fullAddress AS address,
        CASE WHEN brand = 'NA' OR brand IS NULL THEN poi_name ELSE brand END AS company,
        CASE 
            WHEN LENGTH(REGEXP_REPLACE(phone_number, '[^0-9]', '')) = 11 
                AND LEFT(REGEXP_REPLACE(phone_number, '[^0-9]', ''), 1) = '1' 
                THEN RIGHT(REGEXP_REPLACE(phone_number, '[^0-9]', ''), 10)
        ...
        END AS phone_number,
        business_status,
        COALESCE(COALESCE(COALESCE(COALESCE(tier5_naics_code,tier4_naics_code),tier3_naics_code), tier2_naics_code), tier1_naics_code)  AS NAICS,
        NULL AS SIC,
        LOWER(REGEXP_REPLACE(REGEXP_REPLACE(LOWER(CASE WHEN brand = 'NA' OR brand IS NULL THEN poi_name ELSE brand END), words_to_remove, ''),r'[^\w]', '')) AS clean_company,
        CASE 
            -- NAICS matches (Medical, Legal, Accounting, Engineering, Therapy, etc.)
            WHEN COALESCE(COALESCE(COALESCE(COALESCE(tier5_naics_code,tier4_naics_code),tier3_naics_code), tier2_naics_code), tier1_naics_code) IN (
            '621111', '621112', '621210', '621310', '621320', '621330', '621340', '621391', '621399', '621420', '621498', '621999', 
            '541110', '541120', '541199',  
            '541211', '541219',  
            '541310', '541320', '541330',  
  ...
  END AS is_professional_office
    FROM 
        `clgx-idap-bigquery-int-278e.land_property_commercial_prefill.cp_firmographics_echo_clip_v2_1`  WHERE business_status NOT IN('CLOSED_PERMANENTLY')
),
equifax AS (
    SELECT
        'equifax' AS source_table,
        CAST(efx_id AS STRING) as rec_id,
        CONCAT(efx_name,'equifax',NULLIF(CASE 
        ...
        LOWER(REGEXP_REPLACE(REGEXP_REPLACE(LOWER(T1.efx_name), words_to_remove, ''),r'[^\w]', '')) AS clean_company,
        CASE 
            -- NAICS matches (Medical, Legal, Accounting, Engineering, Therapy, etc.)
...
bigrams_table AS (
    SELECT 
        ...
        CONCAT(company,source_table,COALESCE(phone_number,'0')) AS uid,
        ARRAY(
...
--- EA Table
    ...
    SELECT DISTINCT
        ...
        COUNT(DISTINCT CASE WHEN source_table_2 = 'db_usa' AND LOWER(company_1) <> LOWER(company_2) AND address_id_1 = address_id_2 AND (phone_1 = phone_2 OR (phone_1 IS NULL OR phone_2 IS NULL))AND jaccard_index >= 0.5 THEN company_2 END) AS S3_count_dbusa,
        COUNT(DISTINCT CASE WHEN source_table_2 = 'equifax' AND LOWER(company_1) <> LOWER(company_2) AND address_id_1 = address_id_2 AND (phone_1 = phone_2 OR (phone_1 IS NULL OR phone_2 IS NULL)) AND jaccard_index >= 0.5 THEN company_2 END) AS S3_count_efx,
        COUNT(DISTINCT CASE WHEN source_table_2 = 'echo_analytics' AND LOWER(company_1) <> LOWER(company_2) AND address_id_1 = address_id_2 AND (phone_1 = phone_2 OR (phone_1 IS NULL OR phone_2 IS NULL)) AND jaccard_index >= 0.5 THEN company_2 END) AS S3_count_echo,
...
SELECT 
    ...
    T1.NAICS,
    T1,SIC,
    CASE WHEN A.S1_count_equifax > 0 THEN 'Y' ELSE 'N' END AS S1_Itself,
    CASE WHEN A.S1_count_db_usa > 0 THEN 'Y' ELSE 'N' END AS S1_dbusa,
    CASE WHEN A.S1_count_echo_analytics > 0 THEN 'Y' ELSE 'N' END AS S1_echo,

    CASE WHEN A.S2_count_equifax > 0 THEN 'Y' ELSE 'N' END AS S2_Itself,
    CASE WHEN A.S2_count_db_usa > 1 THEN 'Y' ELSE 'N' END AS S2_dbusa,
    CASE WHEN A.S2_count_echo_analytics > 1 THEN 'Y' ELSE 'N' END AS S2_echo,

    CASE WHEN A.S3_count_efx > 0 THEN 'Y' ELSE 'N' END AS S3_1_Itself,
    CASE WHEN A.S3_count_dbusa > 0 THEN 'Y' ELSE 'N' END AS S3_1_dbusa,
    CASE WHEN A.S3_count_echo > 0 THEN 'Y' ELSE 'N' END AS S3_1_echo,

    CASE WHEN A.S4_count_equifax > 0 THEN 'Y' ELSE 'N' END AS S4_Itself,
    CASE WHEN A.S4_count_db_usa > 0 THEN 'Y' ELSE 'N' END AS S4_dbusa,
    CASE WHEN A.S4_count_echo_analytics > 0 THEN 'Y' ELSE 'N' END AS S4_echo,

    CASE WHEN A.S5_count_equifax > 0 THEN 'Y' ELSE 'N' END AS S5_Itself,
    CASE WHEN A.S5_count_db_usa > 1 THEN 'Y' ELSE 'N' END AS S5_dbusa,
    CASE WHEN A.S5_count_echo_analytics > 1 THEN 'Y' ELSE 'N' END AS S5_echo,

FROM consolidated_sources T1
