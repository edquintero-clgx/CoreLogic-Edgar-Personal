...
WITH db_usa AS (
        ...
        NULLIF(LOWER(REGEXP_REPLACE(REGEXP_REPLACE(LOWER(company), r'\b(llc|inc|corp|shoppingcntr|center|plaza|limited|ltd|company|co|co.|westbrook|jrs|corporation|church)\b', ''),r'[^\w]', '')),'') AS clean_company,
        CASE 
            WHEN naics_1 IN (
...
SELECT 
    ...
    CASE WHEN A.S3_1_count_efx > 0 THEN 'Y' ELSE 'N' END AS S3_1_Itself,
    CASE WHEN A.S3_1_count_dbusa > 0 THEN 'Y' ELSE 'N' END AS S3_1_dbusa,
    CASE WHEN A.S3_1_count_echo > 0 THEN 'Y' ELSE 'N' END AS S3_1_echo,

    CASE WHEN A.S3_count_equifax > 0 THEN 'Y' ELSE 'N' END AS S3_Itself,
    CASE WHEN A.S3_count_db_usa > 0 THEN 'Y' ELSE 'N' END AS S3_dbusa,
...
/*
SELECT
    SUM(CASE WHEN S1_Itself='Y' THEN 1 ELSE 0 END) AS S1_Itself_totals,
    SUM(CASE WHEN S1_dbusa='Y' THEN 1 ELSE 0 END) AS S1_dbusa_totals,
    SUM(CASE WHEN S1_echo='Y' THEN 1 ELSE 0 END) AS S1_echo_totals,

    SUM(CASE WHEN S2_Itself='Y' THEN 1 ELSE 0 END) AS S2_Itself_totals,
    SUM(CASE WHEN S2_dbusa='Y' THEN 1 ELSE 0 END) AS S2_dbusa_totals,
    SUM(CASE WHEN S2_echo='Y' THEN 1 ELSE 0 END) AS S2_echo_totals,

    SUM(CASE WHEN S3_Itself='Y' THEN 1 ELSE 0 END) AS S3_Itself_totals,
    SUM(CASE WHEN S3_dbusa='Y' THEN 1 ELSE 0 END) AS S3_dbusa_totals,
    SUM(CASE WHEN S3_echo='Y' THEN 1 ELSE 0 END) AS S3_echo_totals,

    SUM(CASE WHEN S4_Itself='Y' THEN 1 ELSE 0 END) AS S4_Itself_totals,
    SUM(CASE WHEN S4_dbusa='Y' THEN 1 ELSE 0 END) AS S4_dbusa_totals,
    SUM(CASE WHEN S4_echo='Y' THEN 1 ELSE 0 END) AS S4_echo_totals,
